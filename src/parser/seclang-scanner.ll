...
CONFIG_SEC_UPDATE_ACTION_BY_TAG         (?i:SecRuleUpdateActionByTag)
...
{CONFIG_SEC_UPDATE_ACTION_BY_TAG}[ \t]+["]{FREE_TEXT_NEW_LINE}["]       { BEGIN(TRANSACTION_FROM_OPERATOR_TO_ACTIONS); return p::make_CONFIG_SEC_RULE_UPDATE_ACTION_BY_TAG(parserSanitizer(find_separator(yytext)), *driver.loc.back()); }
{CONFIG_SEC_UPDATE_ACTION_BY_TAG}[ \t]+{FREE_TEXT_SPACE_COMMA_QUOTE}    { BEGIN(TRANSACTION_FROM_OPERATOR_TO_ACTIONS); return p::make_CONFIG_SEC_RULE_UPDATE_ACTION_BY_TAG(parserSanitizer(find_separator(yytext)), *driver.loc.back()); }
